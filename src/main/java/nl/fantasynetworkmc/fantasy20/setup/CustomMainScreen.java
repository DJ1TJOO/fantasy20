...
public class CustomMainScreen extends Screen {
	...
	public static final RenderSkyboxCube PANORAMA_RESOURCES = new RenderSkyboxCube(new ResourceLocation("textures/gui/title/background/panorama"));
	   ...
	   @Nullable
	   private CustomMainScreen.WarningDisplay openGLWarning1;
	   ...
	   public CustomMainScreen(MainMenuScreen mainScreen) {
	      this(false, mainScreen);
	   }

	   public CustomMainScreen(boolean fadeIn, MainMenuScreen mainScreen) {
	      super(new StringTextComponent("dsafasdf"));
	      ...
	      if (!GLX.supportsOpenGL2() && !GLX.isNextGen()) {
	         this.openGLWarning1 = new CustomMainScreen.WarningDisplay((new TranslationTextComponent("title.oldgl.eol.line1")).applyTextStyle(TextFormatting.RED).applyTextStyle(TextFormatting.BOLD), (new TranslationTextComponent(""
	         		+ ".oldgl.eol.line2")).applyTextStyle(TextFormatting.RED).applyTextStyle(TextFormatting.BOLD), "https://help.mojang.com/customer/portal/articles/325948?ref=game");
	      }
	   ...
	   protected void init() {
	      ...
	      modUpdateNotification = net.minecraftforge.client.gui.NotificationModUpdateScreen.init(mainScreen, modbutton);
	   ...
	   public void render(int p_render_1_, int p_render_2_, float p_render_3_) {
	      ...
	      if ((l & -67108864) != 0) {
	         ...
	         net.minecraftforge.client.ForgeHooksClient.renderMainMenu(mainScreen, this.font, this.width, this.height);
	         if (this.splashText != null) {
	   ...
	   class WarningDisplay {
	      ...
	      public void init(int yIn) {
	         int i = CustomMainScreen.this.font.getStringWidth(this.firstLine.getString());
	         this.secondLineWidth = CustomMainScreen.this.font.getStringWidth(this.secondLine.getString());
	         int j = Math.max(i, this.secondLineWidth);
	         this.left = (CustomMainScreen.this.width - j) / 2;
	         this.top = yIn - 24;
	      ...
	      public void render(int alpha) {
	         ...
	         CustomMainScreen.this.drawString(CustomMainScreen.this.font, this.firstLine.getFormattedText(), this.left, this.top, 16777215 | alpha);
	         CustomMainScreen.this.drawString(CustomMainScreen.this.font, this.secondLine.getFormattedText(), (CustomMainScreen.this.width - this.secondLineWidth) / 2, this.top + 12, 16777215 | alpha);
	      }
	      ...
	      public boolean mouseClicked(double mouseX, double p_223418_3_) {
	         if (!StringUtils.isNullOrEmpty(this.onClickURL) && mouseX >= (double)this.left && mouseX <= (double)this.right && p_223418_3_ >= (double)this.top && p_223418_3_ <= (double)this.bottom) {
	            CustomMainScreen.this.minecraft.displayGuiScreen(new ConfirmOpenLinkScreen((p_223421_1_) -> {
	               if (p_223421_1_) {
	                  Util.getOSType().openURI(this.onClickURL);
	               }

	               CustomMainScreen.this.minecraft.displayGuiScreen(CustomMainScreen.this);
